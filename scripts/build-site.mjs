...
<p>This is <strong>not</strong> the official js-framework-benchmark harness (<code>webdriver-ts</code>) - no CPU throttling, no devtools tracing, no other frameworks measured alongside it. It exists to catch jq79's own regressions release over release, not to rank against React or Svelte. See <a href="${REPO_URL}/tree/main/${BENCHMARK_DIR}#submitting-upstream">the framework directory's README</a> for what it would take to submit this for a real cross-framework comparison.</p>
